/***********************************************
  ...
  Tumblr.lightbox:
  The heavy lifting to create a lightbox and plugin to Tumblr's architecture.
...
Tumblr.replaceIfFlash = function(version, element_id, replacement_string) {
    if (Tumblr.flashVersion() >= version) {
        document.getElementById(element_id).innerHTML = replacement_string
    }
};
Tumblr.renderVideo = function(element_id, src, width, height, flashvars) {
    ...
    if (is_iphone) {
        ...
        replaceIfFlash(10, element_id, '<embed type="application/x-shockwave-flash" src="http://assets.tumblr.com/swf/video_player.swf?22" bgcolor="#000000" quality="high" class="video_player" wmode="transparent" allowfullscreen="true" height="' + height + '" width="' + width + '" flashvars="file=' + encodeURIComponent(src) + (flashvars ? "&amp;" + flashvars : "") + '"></embed>')
    }
};
